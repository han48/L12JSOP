@component($typeForm, get_defined_vars())
    <div data-controller="attach" class="attach" data-attach-name-value="{{ $name }}"
        data-attach-size-value="{{ $maxSize }}" data-attach-count-value="{{ $maxCount }}"
        data-attach-loading-value="0" data-attach-attachment-value='@json($value ?? [])'
        data-attach-storage-value="{{ $storage ?? 'public' }}" data-attach-path-value="{{ $path }}"
        ...
        data-attach-error-size-value="{{ $errorMaxSizeMessage }}" data-attach-error-type-value="{{ $errorTypeMessage }}"
        data-action="
        ...
        <div data-target="attach.preview" class="row row-cols-4 row-cols-lg-6 gy-3 sortable-dropzone">
            <div class="col order-last attach-file-uploader" data-attach-target="container">
                <label for="{{ $id }}"
                    class="border rounded bg-light attach-image-placeholder pointer-event h-100">
                    <input class="form-control d-none" type="file" data-attach-target="files"
                        data-action="change->attach#selectFiles" disabled {{ $attributes }}>
        ...
        <template data-attach-target="template">
            <div class="pip col position-relative">
                ...
                <img class="attach-image rounded border user-select-none overflow-hidden" src="{url}"
                    title="{original_name}" />
                ...
                <button class="btn-close rounded-circle bg-white border shadow position-absolute end-0 top-0" type="button"
                    data-action="click->attach#remove" data-index="{id}"></button>
            </div>
        </template>
