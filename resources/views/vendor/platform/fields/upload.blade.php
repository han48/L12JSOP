@component($typeForm, get_defined_vars())
    <div data-controller="upload" data-upload-storage="{{ $storage ?? config('platform.attachment.disk', 'public') }}"
        data-upload-name="{{ $name }}" data-upload-id="dropzone-{{ $id }}"
        data-upload-data='@json($value)' data-upload-groups="{{ $attributes['groups'] ?? '' }}"
        data-upload-multiple="{{ $attributes['multiple'] }}" data-upload-parallel-uploads="{{ $parallelUploads }}"
        data-upload-max-file-size="{{ $maxFileSize }}" data-upload-max-files="{{ $maxFiles }}"
        data-upload-timeout="{{ $timeOut }}" data-upload-accepted-files="{{ $acceptedFiles }}"
        data-upload-resize-quality="{{ $resizeQuality }}" data-upload-resize-width="{{ $resizeWidth }}"
        data-upload-is-media-library="{{ $media }}" data-upload-close-on-add="{{ $closeOnAdd }}"
        data-upload-resize-height="{{ $resizeHeight }}" data-upload-path="{{ $attributes['path'] ?? '' }}">
        <div id="dropzone-{{ $id }}" class="dropzone-wrapper">
            ...
            <div class="visual-dropzone sortable-dropzone dropzone-previews">
                ...
                @if ($media)
                    <div class="dz-message dz-preview dz-processing dz-image-preview" data-action="click->upload#openMedia">
                        <div class="bg-light d-flex justify-content-center align-items-center border r-2x"
                            style="min-height: 112px;">
                            <div class="px-2 py-4">
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body p-4">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" data-upload-target="alt" maxlength="255"
                                    placeholder="{{ __('Alternative text') }}">
                            </div>
                            <div class="form-group">
                                <label>{{ __('Description') }}</label>
                                <textarea class="form-control no-resize" data-upload-target="description" placeholder="{{ __('Description') }}"
                                ...
                                <div class="row justify-content-center">
                                    ...
                                    <div class="col-md-12">
                                        <div class="form-group">
                                            ...
                                            <input type="search" data-upload-target="search"
