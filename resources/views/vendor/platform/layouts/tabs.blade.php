<div class="mb-3" data-controller="tabs" data-tabs-slug="{{ $templateSlug }}" data-tabs-active-tab="{{ $activeTab }}">
            ...
            @foreach ($manyForms as $name => $tab)
                <li class="nav-item" role="presentation">
                    <a @class([
                        ...
                        id="button-tab-{{ sha1($templateSlug . $name) }}" aria-selected="false" role="tab"
                        data-bs-toggle="tab">
                        {!! $name !!}
                    </a>
                </li>
    ...
    <section class="tab-content">
        @foreach ($manyForms as $name => $forms)
            <div role="tabpanel" id="tab-{{ sha1($templateSlug . $name) }}" @class([
                'tab-pane',
                'active' => $activeTab === $name || ($loop->first && is_null($activeTab)),
            ])>
