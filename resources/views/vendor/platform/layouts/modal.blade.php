@push('modals-container')
    <div class="modal fade center-scale {{ $type }}" id="screen-modal-{{ $key }}" role="dialog"
        aria-labelledby="screen-modal-{{ $key }}" data-controller="modal" data-modal-slug-value="{{ $templateSlug }}"
        data-modal-url-value="{{ $deferredRoute }}"
        @if (!empty($deferrerParams)) data-modal-parameters-value='@json($deferrerParams)' @endif
        data-modal-open-value="{{ var_export($open) }}" {{ $staticBackdrop ? 'data-bs-backdrop=static' : '' }}>
        <div class="modal-dialog modal-fullscreen-md-down {{ $size }}" role="document"
            id="screen-modal-type-{{ $key }}">
            <form class="modal-content" action="{{ $method }}" id="screen-modal-form-{{ $key }}"
                method="post" enctype="multipart/form-data" data-controller="form" data-action="form#submit">
                ...
                <div class="modal-footer">
                    ...
                    @empty($commandBar)
                        @if (!$withoutApplyButton)
                            <button type="submit" id="submit-modal-{{ $key }}" data-turbo="{{ var_export($turbo) }}"
